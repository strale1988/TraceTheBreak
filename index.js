 ...
 *   GET    /server-fetch?path=<key>  (SERVER_READ_SECRET-gated, trusted server reads —
 *                                      e.g. the utility-notify edge function embedding
 *                                      report photos into PDFs)
 */
...
// Separate from ADMIN_MIGRATION_SECRET on purpose: that one is meant to be
// deleted from the dashboard once the R2 migration is done, but this route
// needs to stay live permanently — it's how the utility-notify Supabase
// edge function reads raw photo bytes to embed in PDF reports. Set via
// `wrangler secret put SERVER_READ_SECRET`.
function requireServerReadSecret(request, env) {
  if (!env.SERVER_READ_SECRET) return false;
  const provided = request.headers.get('x-server-secret') || '';
  return safeEqual(provided, env.SERVER_READ_SECRET);
}
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // ---- GET /server-fetch?path=... : raw object bytes for trusted server callers ----
      // Not for browsers. Used by the Supabase edge function that builds
      // utility-company PDF reports, to pull a report photo's bytes so it
      // can embed them in the PDF. Gated by SERVER_READ_SECRET (see above),
      // never by a user JWT — there's no logged-in user on that side.
      if (url.pathname === '/server-fetch' && request.method === 'GET') {
        if (!requireServerReadSecret(request, env)) return err(401, 'not authorized', origin);
        const path = url.searchParams.get('path');
        if (!path) return err(400, 'missing path', origin);
        const obj = await env.PHOTOS_BUCKET.get(path);
