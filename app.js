...
// Same green used for utility-company POI markers on the map (see
// .company-marker-icon-wrap in style.css) — reused for the contacts modal
// headers so they visually match the pins they're opened from, instead of
// each one picking a different (category or "no color") header background.
const CONTACTS_HEADER_COLOR = '#2f8f5b';
...
// The current user's own pending status vote per report (report_id ->
// suggested_status), so the timeline's vote action can show "already
// voted" and let a mis-tap be undone instead of just re-casting. Separate
// from voteProgressByReport, which only holds aggregate points, not who
// cast them. Only ever populated for the report(s) currently open in the
// detail modal — see loadMyVoteForReport.
let myVoteByReport = new Map();
...
const USER_LEVELS = [
  ...
  { level:2, threshold:20,  weight:1.5, nameEn:'Reporter',              nameSr:'Reporter' },
  { level:3, threshold:50,  weight:2,   nameEn:'Inspector',             nameSr:'Inspektor' },
  { level:4, threshold:100, weight:2.5, nameEn:'Guardian',              nameSr:'Čuvar' },
  { level:5, threshold:180, weight:3,   nameEn:'Community Hero',        nameSr:'Heroj zajednice' },
];
...
const BADGES = [
  ...
  { id:'reporter',       icon:'badge-reporter.png',       nameEn:'Reporter',       nameSr:'Reporter',
    descEn:'Reach Level 2: earn 20 civic points.',                     descSr:'Dostignite Nivo 2: osvojite 20 građanskih poena.',
    check: p => (p && p.total_contributions || 0) >= 20 },
  { id:'inspector',      icon:'badge-inspector.png',      nameEn:'Inspector',      nameSr:'Inspektor',
    descEn:'Reach Level 3: earn 50 civic points.',                     descSr:'Dostignite Nivo 3: osvojite 50 građanskih poena.',
    check: p => (p && p.total_contributions || 0) >= 50 },
  { id:'guardian',       icon:'badge-guardian.png',       nameEn:'Guardian',       nameSr:'Čuvar',
    descEn:'Reach Level 4: earn 100 civic points.',                    descSr:'Dostignite Nivo 4: osvojite 100 građanskih poena.',
    check: p => (p && p.total_contributions || 0) >= 100 },
  { id:'community_hero', icon:'badge-community-hero.png', nameEn:'Community Hero', nameSr:'Heroj zajednice',
    descEn:'Reach Level 5: earn 180 civic points.',                    descSr:'Dostignite Nivo 5: osvojite 180 građanskih poena.',
    check: p => (p && p.total_contributions || 0) >= 180 },
...
// The Notifications modal is now purely the personal inbox (messages/alerts
// addressed to the current admin/user) — the admin-only moderation queue
// ("Waiting for review") and the broadcast/direct-message composer live in
// the Admin Panel instead, so the two surfaces stop showing the same items.
...
function renderUserNotifications() {
  ...
  // Improvement #1: consecutive admin broadcast/direct messages sent the
  // same day collapse into a single digest card instead of flooding the
  // inbox one row per message — the admin still sends them individually
  // from the compose panel, but the recipient sees a digest, not a flood.
  // userNotificationsCache is already sorted newest-first, so grouping
  // adjacent same-day admin messages is enough; nothing else is reordered.
  const groups = [];
  userNotificationsCache.forEach(n => {
    const isAdminMsg = n.notif_type === 'admin_message' || n.sender_is_admin;
    const dayKey = isAdminMsg ? new Date(n.created_at).toDateString() : null;
    const last = groups[groups.length - 1];
    if (isAdminMsg && last && last.dayKey === dayKey) {
      last.items.push(n);
    } else {
      groups.push({ dayKey, items: [n] });
    }
  });

  listEl.innerHTML = groups.map(g => g.items.length === 1 ? renderNotificationItemHtml(g.items[0]) : renderNotificationGroupHtml(g)).join('');
}

function renderNotificationItemHtml(n) {
  const reportId = n.report_id || n.related_report_id || null;
  return `
    <div class="notification-item${n.is_read ? '' : ' unread'}" onclick="handleNotificationItemClick('${n.id}', ${reportId ? `'${reportId}'` : 'null'})" style="cursor:pointer;">
...
let expandedNotificationGroups = new Set();

function renderNotificationGroupHtml(g) {
  const groupKey = 'grp-' + g.dayKey.replace(/[^a-z0-9]+/gi, '-');
  const expanded = expandedNotificationGroups.has(groupKey);
  const anyUnread = g.items.some(n => !n.is_read);
  const dateLabel = formatDate(g.items[0].created_at);
  if (!expanded) {
    const summary = tOr('notificationDigestSummary', '{count} messages from Admin').replace('{count}', String(g.items.length));
    return `
    <div class="notification-item${anyUnread ? ' unread' : ''}" style="cursor:pointer;" onclick="toggleNotificationGroup('${groupKey}')">
      <div class="notification-item-top">
        <span class="notification-item-sender">${escapeHtml(t('notificationAdminTag'))}</span>
        ${anyUnread ? '<span class="notification-item-dot"></span>' : ''}
      </div>
      <div class="notification-item-message">${escapeHtml(summary)}</div>
      <div class="notification-item-meta">${escapeHtml(dateLabel)} · ${escapeHtml(tOr('notificationDigestExpandHint', 'Tap to view all'))}</div>
    </div>`;
  }
  return `
    <div class="notification-digest-group">
      <div class="notification-item-meta" style="cursor:pointer;padding:0 2px 6px;" onclick="toggleNotificationGroup('${groupKey}')">${escapeHtml(tOr('notificationDigestCollapseHint', '▲ Collapse {count} messages').replace('{count}', String(g.items.length)))}</div>
      ${g.items.map(renderNotificationItemHtml).join('')}
    </div>`;
}

function toggleNotificationGroup(groupKey) {
  if (expandedNotificationGroups.has(groupKey)) expandedNotificationGroups.delete(groupKey);
  else expandedNotificationGroups.add(groupKey);
  renderUserNotifications();
}
...
// Notifications stay visually "new" (accent border + dot) and keep counting
// toward the badge until the admin/user actually taps that specific item —
// just opening the inbox is no longer enough to clear them.
async function handleNotificationItemClick(id, reportId) {
  await markSingleNotificationRead(id);
  if (reportId) await openNotificationReportTarget(reportId);
}

async function markSingleNotificationRead(id) {
  const n = userNotificationsCache.find(x => x.id === id);
...
function startNotificationInboxAutoRefresh() {
  ...
  notificationInboxRefreshIntervalId = setInterval(() => {
    if (!isNotificationModalOpen()) { stopNotificationInboxAutoRefresh(); return; }
...
// t() falls back to returning the key itself when no translation exists in
// any language file, so a plain `t(k) || fallback` never actually fires.
// This is for brand-new keys that haven't been added to languages/*.json
// yet — it shows a real word instead of the raw key until a translator adds
// the proper string.
function tOr(k, fallbackText, langOverride) {
  const val = t(k, langOverride);
  return val === k ? fallbackText : val;
}
...
function applyLang(){
  ...
  if (currentProfile && currentProfile.is_admin && document.getElementById('adminPanelModal').style.display !== 'none') {
    ...
    renderUcList();
    renderNotificationComposePanel();
...
function showSettingsModal() {
  ...
  const oldFixedToggleEl = document.getElementById('oldFixedToggleInput');
  if (oldFixedToggleEl) oldFixedToggleEl.checked = showOldFixedReports;
  renderStatusFilterSettings();
  initSoundSettingsUi();
...
function hideAdminPanelModal() {
  ...
  stopWaitingListAutoRefresh();
  closeOverlay('adminPanelModal');
}

function renderNotificationComposePanel() {
...
function pickReportPhotoSource() {
  return new Promise(resolve => {
    ...
    inner.innerHTML = `
      ...
      <button type="button" class="generic-modal-close" style="margin-top:10px;" onclick="closePhotoSourceModal(null)">${escapeHtml(t('cancelBtn'))}</button>`;
    overlay.style.display = 'flex';
...
// Whole categories that never get an "in progress" phase at all —
// Parking (either the car's gone or it isn't) and Suggestion (feedback
// items are received/considered, never "worked on" over time). Neither
// has any subcategories, so there's nothing more granular to check for
// them.
const CATEGORIES_SKIPPING_IN_PROGRESS = new Set(['Parking', 'Suggestion']);

// Some categories mix genuine repair work (which benefits from an
// "in progress" phase — someone is out there fixing it) with
// enforcement/violation-style reports that are really just binary,
// resolved-or-not — same shape as Parking/Suggestion above, just scoped
// to one subcategory instead of the whole category. E.g. Forest still
// wants "in progress" for a fallen tree blocking a trail, but not for a
// report of illegal logging (that's an enforcement matter, not a
// multi-day fix).
const SUBCATEGORIES_SKIPPING_IN_PROGRESS = {
  Waste:         new Set(['dumping']),
  Forest:        new Set(['illegal_logging', 'illegal_dumping']),
  GreenSpaces:   new Set(['illegal_dumping']),
  FarmersMarket: new Set(['illegal_vendor', 'overpricing']),
};

function categorySkipsInProgress(category, subcategory) {
  if (CATEGORIES_SKIPPING_IN_PROGRESS.has(category)) return true;
  const subSkips = SUBCATEGORIES_SKIPPING_IN_PROGRESS[category];
  return !!(subSkips && subcategory && subSkips.has(subcategory));
}

// "fixed" no longer waits on "in progress" being reached first, for any
// category — real-world crews often just fix something without anyone
// ever logging it as "in progress" first, and forcing that step blocked a
// perfectly normal outcome. This used to gate on report.status/category
// (see git history for the old fixedBlockedBeforeInProgress), but the
// underlying cast_status_vote RPC already makes running both votes at
// once safe: whichever suggested_status reaches the threshold first wins
// and the RPC deletes *all* pending votes for the report right after
// (not just the ones for the status that won), so a stale "in progress"
// tally can never linger on a report that just got voted straight to
// "fixed". Nothing else needs to change here — "in progress" itself is
// still fully live and useful (it's real signal that a category with an
// actual in-progress phase is being worked on, and other features —
// company-notify timing, admin "fast resolves" stats, the owner
// fast-track button — all key off it) — it's just no longer a mandatory
// gate in front of "fixed".
...
// Fixed reports stay useful on the map for a while (confirming the fix
// worked, spotting duplicates, etc.) but lose relevance the longer they
// sit — an old "fixed" pin is mostly just clutter next to the reports
// that still need attention. Past this many days, a fixed report is
// hidden from the map by default; the settings toggle below brings them
// back on demand instead of deleting anything. This only affects what's
// drawn on the map/heatmap — the report itself, its history, and its
// detail page are untouched either way.
const OLD_FIXED_REPORT_DAYS = 15;

const SHOW_OLD_FIXED_STORAGE_KEY = 'ttb_show_old_fixed';
function getShowOldFixedPref() {
  try { return localStorage.getItem(SHOW_OLD_FIXED_STORAGE_KEY) === '1'; } catch (e) { return false; }
}
let showOldFixedReports = getShowOldFixedPref();
function setShowOldFixedReports(enabled) {
  showOldFixedReports = !!enabled;
  try { localStorage.setItem(SHOW_OLD_FIXED_STORAGE_KEY, showOldFixedReports ? '1' : '0'); } catch (e) {}
  syncMarkers(true);
  refreshHeatLayer();
}
// Whether this particular report should count against the "old fixed"
// cutoff — only ever applies to fixed reports; anything else (or a fixed
// report missing fixed_at, which shouldn't normally happen now that it's
// part of the lean columns, but better to fail open than hide a report
// over missing data) is always visible on its own age merits.
function isFixedReportRecent(report) {
  if (!report || report.status !== 'fixed' || !report.fixed_at) return true;
  const ageMs = Date.now() - new Date(report.fixed_at).getTime();
  return ageMs < OLD_FIXED_REPORT_DAYS * 24 * 60 * 60 * 1000;
}
function isFixedAgeVisible(report) {
  return showOldFixedReports || isFixedReportRecent(report);
}
...
// --- Accidental-duplicate photo detection -------------------------------
// Same idea as the Esri no-data-tile detector further up this file: decode
// down to a tiny fixed grid and compare pixels rather than trusting a byte
// hash (which would miss a re-compressed or re-cropped copy of the exact
// same shot). This is specifically an "oops, picked the same photo twice"
// safeguard — e.g. the same file chosen again for the after-photo, or
// tapped twice into the gallery — not a general similarity search, so the
// threshold below is kept tight on purpose: it should only ever fire on
// images that are genuinely (near-)identical, never on two different
// photos of a similar-looking pole or street corner.
const PHOTO_HASH_GRID = 8;                // 8x8 -> 64-bit average hash
const PHOTO_DUPLICATE_HAMMING_MAX = 6;    // out of 64 bits; tight on purpose

// Computes a 64-bit average-hash (as a BigInt) from an image source. Accepts
// a Blob/File (freshly picked, not yet uploaded) or anything else
// drawImage() accepts (HTMLImageElement, ImageBitmap) — used to hash photos
// that are already sitting on the page as <img> thumbnails.
async function computePhotoHash(source) {
  let bitmap = source;
  ...
  try {
    ...
    let hash = 0n;
    for (let i = 0; i < gray.length; i++) hash = (hash << 1n) | (gray[i] > mean ? 1n : 0n);
    return hash;
  } finally {
    if (ownsBitmap && bitmap.close) bitmap.close();
  }
}

function photoHashHammingDistance(a, b) {
...
// Hashes whatever image is already loaded behind an on-page <img> thumbnail
// (main/after timeline photo dots, gallery thumbs) — reuses the bytes the
// browser already fetched instead of re-downloading anything.
async function computePhotoHashFromImgEl(imgEl) {
  if (!imgEl || !imgEl.src || !imgEl.complete || !imgEl.naturalWidth) return null;
  try { return await computePhotoHash(imgEl); } catch (e) { return null; }
}

// Every photo currently visible for a report in its open detail modal:
// the before/after timeline-dot thumbnails plus any community gallery
// thumbs. Deliberately scoped to what's on screen right now rather than
// re-fetching the whole photo history from the server — cheap, and the
// case this guards against (picking the same file twice in one sitting)
// is always against something the person can already see.
async function collectVisibleReportPhotoHashes(reportId) {
  const modal = document.getElementById('reportDetailModal');
  if (!modal || modal.dataset.openReportId !== reportId) return [];
  const imgs = modal.querySelectorAll('.timeline-dot-thumb');
  const hashes = await Promise.all(Array.from(imgs).map(computePhotoHashFromImgEl));
  return hashes.filter(h => h !== null);
}

// Checks a freshly-picked file against the photos already on the report
// and, if it looks like the same shot, asks for confirmation before
// letting the upload proceed. Never hard-blocks: any detection failure
// (odd file, decode error, modal not open) just lets the photo through —
// this is a "catch the obvious mistake" nudge, not a gate.
async function warnIfDuplicateReportPhoto(file, reportId) {
  try {
    const [newHash, existingHashes] = await Promise.all([
      computePhotoHash(file),
      collectVisibleReportPhotoHashes(reportId)
    ]);
    const isDuplicate = existingHashes.some(h => photoHashHammingDistance(newHash, h) <= PHOTO_DUPLICATE_HAMMING_MAX);
    if (!isDuplicate) return true;
    return await themedConfirm(tOr('duplicatePhotoConfirm', 'This looks like a photo that\'s already on this report. Add it anyway?'));
  } catch (e) {
    console.error('Duplicate photo check failed:', e.message || e);
    return true;
  }
}
...
async function addPhotoToReport(reportId) {
  ...
  if (!file) return;
  if (!(await warnIfDuplicateReportPhoto(file, reportId))) return;
...
// Whether the current viewer is allowed to actually see (thumbnail/open)
// a given gallery photo — same rule loadReportGalleryPhotos's default
// filtering uses: approved is public, pending/rejected is uploader+admin
// only. Exposed separately (not just baked into the fetch) so a caller can
// still know a *record* exists — e.g. to render a "Photo added" timeline
// entry — without being allowed to see its actual image.
function isGalleryPhotoViewableByCurrentUser(p) {
  const isAdmin = !!(currentProfile && currentProfile.is_admin);
  const uid = currentSession && currentSession.user && currentSession.user.id;
  return p.photo_status === 'approved' || (!!uid && p.uploader_id === uid) || isAdmin;
}

// includeAll=true skips the visibility filter and returns every gallery
// row for the report (RLS already allows SELECT on all of them — see note
// below) instead of just the ones this viewer may see the image for. Used
// by the timeline, which needs to know a photo was added at all, even one
// it won't render a thumbnail for; the gallery strip itself still calls
// this with includeAll left false, so it only ever displays the images
// this viewer is actually entitled to.
async function loadReportGalleryPhotos(reportId, includeAll) {
  try {
    ...
    return (data || []).filter(isGalleryPhotoViewableByCurrentUser);
  } catch (err) {
...
// Small inline icon-button pair ("approve" / "reject") shown on a
// timeline "Photo added" row while that photo is still pending review —
// this is what used to live in the now-removed "Fotografije zajednice"
// gallery strip (see old renderReportGallery). Admin-only, and only for
// a photo the AI moderation webhook hasn't already approved on its own;
// an approved (or already-rejected) photo shows no buttons here at all.
// Just two plain icons, no labels: check.png to approve, waste.png to
// reject — sits right next to the photo's own timeline row/thumbnail so
// the admin can look at the image and act on it in one place.
function timelinePhotoReviewActionsHtml(isAdmin, status, approveCall, rejectCall) {
  if (!isAdmin || status !== 'pending') return '';
  return `<span class="timeline-photo-review-actions">
    <button type="button" class="timeline-photo-review-btn approve" onclick="event.stopPropagation();${approveCall}" aria-label="${t('photoApproveBtn')}" title="${t('photoApproveBtn')}"><img src="icons/check.png" alt=""></button>
    <button type="button" class="timeline-photo-review-btn reject" onclick="event.stopPropagation();${rejectCall}" aria-label="${t('photoRejectBtn')}" title="${t('photoRejectBtn')}"><img src="icons/waste.png" alt=""></button>
  </span>`;
}
...
async function loadReportStatusVotesForTimeline(reportId) {
  try {
    const { data, error } = await sb.from(REPORT_STATUS_VOTES_TABLE)
      .select('id, suggested_status, created_at')
      .eq('report_id', reportId)
      .order('created_at', { ascending: true });
    if (error) throw error;
    return data || [];
...
// Whether the current viewer is allowed to actually see a report's
// before/after evidence photo — same rule the old dedicated photo
// sections used to gate on: approved needs a signed-in viewer, pending is
// owner/admin only, rejected is admin only. Used to decide whether a
// timeline photo dot gets a real clickable thumbnail or just stays a
// plain camera icon (no thumbnail fetch attempted either way for anyone
// who isn't allowed to see it).
function isReportPhotoViewableByCurrentUser(report, which) {
  const path = which === 'after' ? report.after_photo_path : report.photo_path;
  const status = which === 'after' ? report.after_photo_status : report.photo_status;
  if (!path || !currentSession) return false;
  const isOwner = currentSession.user.id === report.owner_id;
  const isAdmin = !!(currentProfile && currentProfile.is_admin);
  return status === 'approved' || (status === 'pending' && (isOwner || isAdmin)) || (status === 'rejected' && isAdmin);
}

// Fetches a thumbnail for a timeline photo dot and swaps it in once ready
// — the dot starts out as a plain camera icon (see buildTimelineItem) so
// there's something reasonable on screen immediately, then becomes a real
// clickable thumbnail as soon as the signed URL resolves. Silently leaves
// the icon in place on failure.
function resolveTimelinePhotoDot(domId, photoPath) {
  getReportPhotoSignedUrl(photoPath, null, 'thumb').then(url => {
    if (!url) return;
    const dot = document.getElementById(domId);
    if (!dot) return;
    dot.classList.add('timeline-dot-photo');
    dot.innerHTML = `<img class="timeline-dot-thumb" src="${url}" alt="${t('photoViewFullSize') || ''}">`;
    dot.setAttribute('role', 'button');
    dot.setAttribute('tabindex', '0');
    dot.setAttribute('aria-label', t('photoViewFullSize') || '');
    dot.onclick = () => openFullSizeReportPhoto(photoPath);
    dot.onkeydown = (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); openFullSizeReportPhoto(photoPath); } };
  });
}
let timelinePhotoDotSeq = 0;
...
function buildTimelineExtraItem(dateStr, rightHtml, iconSrc, dotId, actionHtml) {
  return buildTimelineEventItem(dateStr, rightHtml, iconSrc, dotId, actionHtml).replace('class="timeline-item ', 'class="timeline-item timeline-extra ');
}

// Small "✕" affixed to the right of an admin-removable timeline-extra row
// (currently: status-vote suggestions and logged contact attempts —
// history that can legitimately be a mistake or noise, unlike the core
// reported/in_progress/fixed stages or a photo, which already has its own
// dedicated delete control in the gallery strip). `this` in the caller's
// onclick is passed through as `btn` so the handler can find its own row
// without a separate id-per-row bookkeeping scheme.
function timelineExtraDeleteBtnHtml(onclickCall) {
  if (!currentProfile || !currentProfile.is_admin) return '';
  const label = escapeHtml(t('deleteBtn') || 'Delete');
  return `<button type="button" class="timeline-extra-delete" onclick="event.stopPropagation();${onclickCall}" aria-label="${label}" title="${label}">✕</button>`;
}

// Deletes one logged status-vote suggestion (the "Predloženo: ..." rows)
// from a report's timeline — admin only. Re-fetches vote progress too,
// since removing a vote can change (or clear) the count shown on the
// timeline's own pending vote action and on the map-popup's vote badge.
async function deleteTimelineStatusVote(voteId, reportId) {
  if (!currentProfile || !currentProfile.is_admin) return;
  if (!confirm(tOr('confirmDeleteTimelineEntry', 'Delete this timeline entry?'))) return;
  try {
    const { error } = await sb.from(REPORT_STATUS_VOTES_TABLE).delete().eq('id', voteId);
    if (error) throw error;
    await loadVoteProgress([reportId]);
    await showReportDetailModal(reportId);
  } catch (err) {
    console.error('Failed to delete timeline vote:', err.message || err);
    toast(tOr('deleteFail', 'Failed to delete'), 'error');
  }
}

// Wipes every outstanding vote for one suggested status on a report in a
// single go — admin only. Sits on the pending "Vote: ..." timeline action
// itself (see buildTimelineStageActionItemHtml), separate from
// deleteTimelineStatusVote above which only removes one already-logged
// "Suggested: ..." row at a time; this is for clearing a stuck/miscast
// tally in one click rather than hunting down each individual vote.
async function clearTimelineStatusVotes(reportId, suggestedStatus) {
  if (!currentProfile || !currentProfile.is_admin) return;
  if (!confirm(tOr('confirmClearVotes', 'Clear all votes for this status?'))) return;
  try {
    const { error } = await sb.from(REPORT_STATUS_VOTES_TABLE).delete().eq('report_id', reportId).eq('suggested_status', suggestedStatus);
    if (error) throw error;
    await loadVoteProgress([reportId]);
    await showReportDetailModal(reportId);
  } catch (err) {
    console.error('Failed to clear timeline votes:', err.message || err);
    toast(tOr('deleteFail', 'Failed to delete'), 'error');
  }
}

// Deletes one logged phone/email contact attempt from a report's
// timeline — admin only, same rationale as deleteTimelineStatusVote.
async function deleteTimelineContactEvent(eventId, reportId) {
  if (!currentProfile || !currentProfile.is_admin) return;
  if (!confirm(tOr('confirmDeleteTimelineEntry', 'Delete this timeline entry?'))) return;
  try {
    const { error } = await sb.from(REPORT_CONTACT_EVENTS_TABLE).delete().eq('id', eventId);
    if (error) throw error;
    await showReportDetailModal(reportId);
  } catch (err) {
    console.error('Failed to delete timeline contact event:', err.message || err);
    toast(tOr('deleteFail', 'Failed to delete'), 'error');
  }
}
...
async function loadDetailTimelineExtras(report) {
  ...
  // {domId, photoPath} pairs for photo events whose dot should become a
  // real clickable thumbnail — resolved only after their HTML has actually
  // landed in the DOM (see the bottom of this function).
  const pendingThumbs = [];
  const isAdmin = !!(currentProfile && currentProfile.is_admin);
  ...
  if (report.photo_uploaded_at) {
    const dotId = `timelinePhotoDot-${++timelinePhotoDotSeq}`;
    const reviewHtml = timelinePhotoReviewActionsHtml(isAdmin, report.photo_status, `approveReportPhoto('${report.id}')`, `rejectReportPhoto('${report.id}')`);
    events.push({ time: report.photo_uploaded_at, html: buildTimelineExtraItem(report.photo_uploaded_at, `<span class="timeline-note">${t('timelinePhotoAddedLabel') || 'Photo added'}</span>`, 'icons/camera.png', dotId, reviewHtml) });
    if (isReportPhotoViewableByCurrentUser(report, 'before')) pendingThumbs.push({ domId: dotId, photoPath: report.photo_path });
  }
  if (report.after_photo_uploaded_at) {
    const dotId = `timelinePhotoDot-${++timelinePhotoDotSeq}`;
    const reviewHtml = timelinePhotoReviewActionsHtml(isAdmin, report.after_photo_status, `approveAfterReportPhoto('${report.id}')`, `rejectAfterReportPhoto('${report.id}')`);
    events.push({ time: report.after_photo_uploaded_at, html: buildTimelineExtraItem(report.after_photo_uploaded_at, `<span class="timeline-note">${t('timelineAfterPhotoAddedLabel') || 'After photo added'}</span>`, 'icons/camera.png', dotId, reviewHtml) });
    if (isReportPhotoViewableByCurrentUser(report, 'after')) pendingThumbs.push({ domId: dotId, photoPath: report.after_photo_path });
  }

  const [galleryPhotos, contactEvents, statusVotes] = await Promise.all([
    // includeAll=true: every gallery photo gets its "Photo added" row on
    // the timeline, same as the main before/after photo does — a pending
    // photo isn't invisible, just its thumbnail is (see below). Previously
    // this called loadReportGalleryPhotos() filtered, which dropped the
    // whole timeline row for anything not-yet-approved instead of just the
    // thumbnail — so an admin (who can see everything) and the reporter
    // viewing their own pending photo without an active session would see
    // a different *number* of timeline entries for the same report.
    loadReportGalleryPhotos(report.id, true),
    loadReportContactEventsForTimeline(report.id),
    loadReportStatusVotesForTimeline(report.id)
  ]);
  (galleryPhotos || []).forEach(p => {
    const dotId = `timelinePhotoDot-${++timelinePhotoDotSeq}`;
    // Pending gallery photo AI hasn't cleared yet → admin gets the same
    // check/waste review icons as the main before/after photo rows. A
    // photo already reviewed (approved/rejected) instead gets a plain "✕"
    // if the current viewer is allowed to remove it outright (the
    // uploader themselves, or an admin) — same rule the old gallery strip
    // used for its delete button.
    const reviewHtml = timelinePhotoReviewActionsHtml(isAdmin, p.photo_status, `approveGalleryPhoto('${p.id}','${report.id}')`, `rejectGalleryPhoto('${p.id}','${report.id}')`);
    const canDelete = !reviewHtml && !!currentSession && (currentSession.user.id === p.uploader_id || isAdmin);
    const deleteHtml = canDelete ? timelineExtraDeleteBtnHtml(`deleteGalleryPhoto('${p.id}','${escapeHtml(p.photo_path)}','${report.id}')`) : '';
    events.push({ time: p.created_at, html: buildTimelineExtraItem(p.created_at, `<span class="timeline-note">${t('timelinePhotoAddedLabel') || 'Photo added'}</span>`, 'icons/camera.png', dotId, reviewHtml || deleteHtml) });
    // Row always shows; the actual thumbnail (and full-size image) is only
    // wired up for viewers allowed to see it — same approved/uploader/admin
    // rule the old gallery strip used.
    if (isGalleryPhotoViewableByCurrentUser(p)) pendingThumbs.push({ domId: dotId, photoPath: p.photo_path });
  });
  contactEvents.forEach(c => {
    const isEmail = c.contact_type === 'email';
    const icon = isEmail ? 'icons/email-sent.png' : 'icons/phone.png';
    const label = isEmail ? (t('timelineContactEmailLabel') || 'Email contact logged') : (t('timelineContactCallLabel') || 'Phone contact logged');
    const deleteHtml = timelineExtraDeleteBtnHtml(`deleteTimelineContactEvent('${c.id}','${report.id}')`);
    events.push({ time: c.created_at, html: buildTimelineExtraItem(c.created_at, `<span class="timeline-note">${label}</span>`, icon, null, deleteHtml) });
  });
  statusVotes.forEach(v => {
    const voteLabel = (t('timelineVoteLabelPrefix') || 'Suggested') + ': ' + escapeHtml(statusLabel(v.suggested_status));
    const deleteHtml = timelineExtraDeleteBtnHtml(`deleteTimelineStatusVote('${v.id}','${report.id}')`);
    events.push({ time: v.created_at, html: buildTimelineExtraItem(v.created_at, `<span class="timeline-note">${voteLabel}</span>`, 'icons/vote.png', null, deleteHtml) });
  });
  ...
  pendingThumbs.forEach(pt => resolveTimelinePhotoDot(pt.domId, pt.photoPath));
}
...
// Admin approve/reject for a single community gallery photo — the gallery
// equivalent of approveReportPhoto/rejectReportPhoto for the main before/
// after photo. Unlike those, this was previously impossible: there was no
// RLS UPDATE policy on report_gallery_photos at all, so a pending gallery
// photo could only ever be reviewed by the AI moderation webhook itself,
// never by a human. Reuses the same photoApproveBtn/photoRejectBtn/
// photoApproved/photoRejected/photoActionFailed strings the main-photo
// flow already uses, rather than new ones.
async function approveGalleryPhoto(id, reportId) {
  try {
    const { data, error } = await sb.from(REPORT_GALLERY_TABLE).update({
    ...
    console.error('Failed to approve gallery photo:', err.message || err);
    toast(t('photoActionFailed'), 'error');
  }
}

async function rejectGalleryPhoto(id, reportId) {
  ...
  try {
    ...
    console.error('Failed to reject gallery photo:', err.message || err);
    toast(t('photoActionFailed'), 'error');
  }
}
...
async function showMunicipalityContactsModal() {
  ...
  try {
    ...
    // nothing verified yet, so the "not verified yet" warning only shows
    ...
    // Municipality-wide stats used to live in the per-company detail modal
    // (opened from a POI); they live here now instead, once per municipality,
    // above the contact list — the company POI modal only shows that
    // company's own stats.
    const muniStats = await getMunicipalityStats(muni.id);
    const muniStatsHtml = buildMuniStatsGridHtml(muniStats);
    const statsSectionHtml = muniStatsHtml
      ? `<div class="detail-section" style="padding-top:20px;">
          <div class="detail-section-title-icon-row"><img class="detail-section-title-icon" src="icons/analytics.png" alt="">${t('muniStatsSectionLabel')}</div>
          ${muniStatsHtml}
        </div>`
      : '';
    const unverifiedNoteHtml = showingUnverified
      ? `<p class="detail-export-hint">${t('municipalityContactsUnverifiedNote')}</p>`
      : '';

    bodyEl.innerHTML = companies.length
      ? statsSectionHtml + unverifiedNoteHtml + renderContactCards(companies, null)
      : statsSectionHtml + `<div class="detail-empty">${t('detailNoContacts')}</div>`;
  } catch (err) {
...
async function showCompanyDetailModal(id) {
  ...
  const catsHtml = buildCompanyCatsHtml(c, 6);

  // Municipality is no longer shown here — it's implicit from which POI/list
  // you opened this from, and now only appears as its own stat section in
  // the main municipality contacts modal instead of being repeated per company.
  body.innerHTML = `
  ...
  // Municipality-wide stats moved to the main municipality contacts modal —
  // this POI-triggered modal only shows this one company's own stats.
  const companyStats = await getUtilityCompanyStats(c.id);
  ...
  if (companyStatsSectionEl && companyStatsHtml) {
    companyStatsSectionEl.innerHTML = `
      <div class="detail-section">
        <div class="detail-section-title-icon-row"><img class="detail-section-title-icon" src="icons/analytics.png" alt="">${t('companyStatsSectionLabel')}</div>
        ${companyStatsHtml}
      </div>`;
  }
}
...
async function loadWaitingListAdmin() {
  ...
  try {
    const { data, error } = await sb.from(TABLE).select('*')
      ...
      .limit(WAITING_LIST_FETCH_LIMIT);
    if (error) throw error;
    // A flagged_for_review report is either a manual admin reject
    // (flag_reason === 'rejected_photo') or an automatic duplicate-photo
    // hit from detect-duplicate-photo (flag_reason starts with
    // 'duplicate_photo:<matched report id>') -- split them into distinct
    // waiting-list reasons so the queue badge and actions match what
    // actually happened instead of always saying "rejected".
    (data || []).forEach(r => addReason(r,
      (r.flag_reason || '').startsWith('duplicate_photo:') ? 'duplicate_photo' : 'rejected_photo',
      r.photo_reviewed_at || r.created_at));
  } catch (err) {
  ...
  renderWaitingList();
  loadContactFreshnessForWaitingList(waitingListCache);
}

// Improvement #5: surfaces whether the municipality contact an admin would
// actually use to escalate a waiting item is verified and recent, right in
// the queue — instead of an admin only discovering a dead/unverified email
// after they've already spent time on the report itself. One batched query
// per refresh (not one per row) since the queue can hold up to
// WAITING_LIST_DISPLAY_LIMIT items.
let waitingListContactCache = new Map(); // municipality_id -> utility_companies rows
const CONTACT_STALE_DAYS = 180;

async function loadContactFreshnessForWaitingList(items) {
  const muniIds = Array.from(new Set(items.map(i => i.report && i.report.municipality_id).filter(id => id != null)));
  if (!muniIds.length) { waitingListContactCache = new Map(); renderWaitingList(); return; }
  try {
    const { data, error } = await sb.from(UTILITY_COMPANIES_TABLE)
      .select('municipality_id, categories, verified, verified_at')
      .in('municipality_id', muniIds);
    if (error) throw error;
    const map = new Map();
    (data || []).forEach(c => {
      if (!map.has(c.municipality_id)) map.set(c.municipality_id, []);
      map.get(c.municipality_id).push(c);
    });
    waitingListContactCache = map;
  } catch (err) {
    console.error('Failed to load contact freshness for waiting list:', err.message);
    waitingListContactCache = new Map();
  }
  renderWaitingList();
}

// Returns a small badge (or '') describing the state of the contact an
// admin would use for this report's category in its municipality: no
// utility company on file at all, one on file but never verified, or one
// verified but stale (verified_at older than CONTACT_STALE_DAYS). A fresh,
// verified contact renders nothing, so the queue only calls out the cases
// that actually need attention.
function contactFreshnessBadgeHtml(report) {
  if (!report || report.municipality_id == null) return '';
  const companies = waitingListContactCache.get(report.municipality_id) || [];
  const matches = companies.filter(c => Array.isArray(c.categories) && c.categories.includes(report.category));
  if (!matches.length) {
    return `<span class="photo-status-badge" style="background:${UI_COLORS.dangerStrong};" title="${escapeHtml(tOr('waitingListNoContactBadgeTip', 'No utility company on file for this category/municipality'))}">${escapeHtml(tOr('waitingListNoContactBadge', 'No contact on file'))}</span>`;
  }
  const verifiedMatches = matches.filter(c => c.verified && c.verified_at);
  if (!verifiedMatches.length) {
    return `<span class="photo-status-badge" style="background:${UI_COLORS.dangerStrong};" title="${escapeHtml(tOr('waitingListUnverifiedContactBadgeTip', 'The contact on file has never been verified by an admin'))}">${escapeHtml(tOr('waitingListUnverifiedContactBadge', 'Contact unverified'))}</span>`;
  }
  const mostRecent = verifiedMatches.reduce((a, b) => new Date(a.verified_at) > new Date(b.verified_at) ? a : b);
  const days = Math.floor((Date.now() - new Date(mostRecent.verified_at).getTime()) / 86400000);
  if (days >= CONTACT_STALE_DAYS) {
    return `<span class="photo-status-badge" style="background:var(--text-muted);" title="${escapeHtml(tOr('waitingListStaleContactBadgeTip', 'Last verified over 6 months ago'))}">${escapeHtml(tOr('waitingListStaleContactBadge', 'Contact stale · {days}d').replace('{days}', String(days)))}</span>`;
  }
  return '';
}
...
function anyAdminQueueModalOpen() {
  const panelEl = document.getElementById('adminPanelModal');
...
function renderWaitingListInto(containerId) {
  ...
  listEl.innerHTML = filtered.map(item => {
    ...
    const badgesHtml = item.reasons.map(reason => {
      const label = reason === 'duplicate_photo'
        ? tOr('queueTypeDuplicatePhoto', 'Possible duplicate')
        : t(WAITING_REASON_LABEL[reason]);
      const bg = reason === 'duplicate_photo' ? UI_COLORS.dangerStrong : 'var(--accent)';
      return `<span class="photo-status-badge" style="background:${bg};">${escapeHtml(label)}</span>`;
    }).join(' ') + contactFreshnessBadgeHtml(r);
    // detect-duplicate-photo (see the add_duplicate_photo_detection
    // migration) writes flag_reason as 'duplicate_photo:<matched report
    // id>' when it auto-flags an exact byte-identical photo re-upload --
    // surface which report it matches, with a jump straight to it, instead
    // of just showing the generic "flagged" badge a manual reject gets.
    const duplicateMatchId = (r.flag_reason || '').startsWith('duplicate_photo:') ? r.flag_reason.split(':')[1] : null;
    const duplicateNoteHtml = duplicateMatchId ? `
        <div class="report-flag-history-item">
          ${escapeHtml(tOr('duplicatePhotoNote', 'Photo exactly matches an existing photo on report {id}').replace('{id}', duplicateMatchId))}
          <button type="button" class="settings-btn" style="padding:2px 8px;font-size:var(--fs-11);margin-top:4px;" onclick="viewReportFromAdminQueue('${duplicateMatchId}')">${escapeHtml(tOr('viewOriginalBtn', 'View original report'))}</button>
        </div>` : '';
    ...
    const actionsHtml = [
      ...
      (item.reasons.includes('rejected_photo') || item.reasons.includes('duplicate_photo')) ? `<button type="button" class="settings-btn" onclick="clearReportFlag('${r.id}')">${t('flagClearBtn')}</button>` : '',
...
// Merges a freshly-updated report row (e.g. the result of a photo
// approve/reject) into the in-memory copy the rest of the app reads from,
// then re-renders the report-detail timeline if it's the one currently
// open — so clicking check.png/waste.png on a timeline photo row updates
// that row immediately, the same way approving from the admin waiting
// list already did for that list.
function patchActiveReportAndRefreshTimeline(reportId, updatedFields) {
  const report = globalActiveData.find(r => r.id === reportId);
  if (report) Object.assign(report, updatedFields);
  refreshDetailTimelineExtras(reportId);
}
...
async function approveReportPhoto(reportId) {
  try {
    ...
    if (!data) throw new Error('no-row-updated');
    toast(t('photoApproved'), 'success');
    patchActiveReportAndRefreshTimeline(reportId, data);
    await loadWaitingListAdmin();
    await loadPinsByWindow();
  } catch (err) {
...
function buildTimelineItem(dateStr, reached, color, labelHtml, iconSrc, rightLabelHtml, dotId) {
  ...
  // dotId lets resolveTimelinePhotoDot find this exact dot later and swap
  // the plain icon for a real photo thumbnail once its signed URL loads.
  const dotIdAttr = dotId ? ` id="${dotId}"` : '';
  // The top row can carry a label on the left (stage name or event note)
  // and, optionally, a username/id on the right (reporterRowHtml or a
  // plain reporter name) — same row, opposite ends. Either side can be
  // omitted; the row itself only renders when there's something to show.
  const hasTopRow = !!(labelHtml || rightLabelHtml);
  const topRowHtml = hasTopRow
    ? `<span class="timeline-label-top">${labelHtml ? `<span class="timeline-label-left">${labelHtml}</span>` : ''}${rightLabelHtml ? `<span class="timeline-label-right">${rightLabelHtml}</span>` : ''}</span>`
    : '';
  return `<div class="timeline-item ${reached ? '' : 'pending'}"${timeAttr}>
    <div class="timeline-line"></div>
    <div class="timeline-dot"${dotIdAttr}${dotStyle}>${iconHtml}</div>
    <div class="timeline-content${hasTopRow ? ' has-label' : ''}">
      ${topRowHtml}
...
function buildTimelineEventItem(dateStr, rightHtml, iconSrc, dotId, actionHtml) {
  return buildTimelineItem(dateStr, true, TIMELINE_EVENT_COLOR, rightHtml, iconSrc, actionHtml, dotId);
}
...
function buildPopupHtml(report) {
  ...
  if (hasFullPower) {
    ...
    const statuses = [
      ...
      && !(s.key === 'in_progress' && categorySkipsInProgress(report.category, report.subcategory)));
    ...
    // "fixed" is votable right alongside "in progress" for every
    // category now, not just the ones that skip in-progress entirely —
    // real fixes sometimes happen without anyone logging the
    // in-progress step first. Safe to run both counts at once: the
    // cast_status_vote RPC clears *all* pending votes for the report
    // the moment either one resolves, so there's never a stale leftover
    // tally sitting on a report that already moved on.
    const statuses = [
      { key: 'reported',    label: t('markReported'),   bg: STATUS_COLORS.reported },
      { key: 'in_progress', label: t('markInProgress'), bg: STATUS_COLORS.in_progress },
      { key: 'fixed',       label: t('markFixed'),      bg: STATUS_COLORS.fixed }
...
const REPORT_DETAIL_ONLY_FIELDS = [
  ...
  // Improvement #4: AI moderation verdicts, needed to build the
  // "Moderation history" section on the detail view (admin-only).
  'photo_moderation_verdict', 'photo_moderation_confidence', 'photo_moderation_reason',
  'after_photo_moderation_verdict', 'after_photo_moderation_confidence', 'after_photo_moderation_reason'
];
...
// No-ops once a report has already been hydrated. Note realtime updates
// (applyRealtimeReportChange) deliver lean broadcast payloads only — they do
// NOT set _detailHydrated, so opening the detail modal for a report that's
// only ever arrived via a live update still comes through here as expected.
...
async function showReportDetailModal(reportId) {
  ...
  await loadMyVoteForReport(reportId);
  ...
  // Category goes in the header title itself; the subcategory (when there
  // is one) now gets its own row underneath instead of being crammed onto
  // the same line as "Category / Subcategory".
  titleEl.textContent = translateCategory(report.category);
  const subcatRowEl = document.getElementById('reportDetailSubcatRow');
  if (subcatRowEl) {
    if (report.subcategory) {
      subcatRowEl.textContent = subcategoryLabel(report.category, report.subcategory);
      subcatRowEl.style.display = '';
    } else {
      subcatRowEl.textContent = '';
      subcatRowEl.style.display = 'none';
    }
  }
  ...
  if (headerEl) {
    headerEl.style.background = categoryColor(report.category);
    headerEl.classList.add('status-colored');
  }

  // Share/copy-link icons used to live in a dedicated "Izvoz" (Export)
  // section at the bottom of the panel; they now sit in the header instead
  // — same buttons, same handlers, just wired here instead of baked into
  // per-report HTML each time.
  const copyLinkBtn = document.getElementById('reportDetailCopyLinkBtn');
  if (copyLinkBtn) {
    copyLinkBtn.title = t('copyLinkBtn');
    copyLinkBtn.setAttribute('aria-label', t('copyLinkBtn'));
    copyLinkBtn.onclick = () => copyReportLinkToClipboard(report.id);
    const copyLinkIcon = document.getElementById('reportDetailCopyLinkIcon');
    if (copyLinkIcon) copyLinkIcon.src = 'icons/copy.png';
  }
  const shareBtn = document.getElementById('reportDetailShareBtn');
  if (shareBtn) shareBtn.onclick = () => shareReport(report.id);

  // The full-power (admin) edit/delete controls move to their own section
  // at the bottom of the panel (buildDetailAdminActionsHtml). A plain
  // owner without full power keeps a lightweight one-tap delete, now as an
  // icon button up in the header instead of a labeled button in the status
  // card.
  ...
  const deleteIconBtn = document.getElementById('reportDetailDeleteIconBtn');
  if (deleteIconBtn) {
    if (canDelete && !hasFullPower) {
      deleteIconBtn.style.display = '';
      deleteIconBtn.title = t('deleteBtn');
      deleteIconBtn.setAttribute('aria-label', t('deleteBtn'));
      deleteIconBtn.onclick = () => deleteReport(report.id);
    } else {
      deleteIconBtn.style.display = 'none';
      deleteIconBtn.onclick = null;
    }
  }

  // The header "add photo" icon button was removed — adding a photo now
  // only happens from the timeline's "+" action (buildTimelineAddPhotoItemHtml
  // / addTimelinePhoto), so there's nothing to wire up here anymore.

  const reporterName = reporterDisplayName(report);

  // "Report an issue with this report" now lives up in the status/priority
  // row (see buildDetailStatusReadonlyHtml) instead of its own section
  // down here — canFlagReport is still needed to know whether to kick off
  // loadReportFlagSection once that row is actually in the DOM.
  ...
  // Before/after evidence photos and community gallery uploads no longer
  // get their own cards — they show up as thumbnails right on the
  // timeline below (see buildDetailTimelineCardHtml / loadDetailTimelineExtras),
  // with a single "+" entry at the end of the timeline for adding a new one.
  const adminActionsHtml = buildDetailAdminActionsHtml(report);
  const moderationHistoryHtml = isAdmin ? buildModerationHistoryHtml(report) : '';

  body.innerHTML = `
    <div class="detail-section">
      ${buildDetailTimelineCardHtml(report, reporterName)}
    </div>
    <div class="detail-section">
    ...
    ${moderationHistoryHtml ? `<div class="detail-section">${moderationHistoryHtml}</div>` : ''}
    ${adminActionsHtml ? `<div class="detail-section">${adminActionsHtml}</div>` : ''}
  `;

  if (isAdmin && moderationHistoryHtml) loadModerationHistoryReviewers(report);
  ...
  // Community gallery photos no longer get their own "Fotografije
  // zajednice" strip/section — every gallery upload (and its admin
  // approve/reject controls, once flagged pending) now shows up as a
  // "Photo added" row right on the timeline below, same as the main
  // before/after photos (see loadDetailTimelineExtras).
...
function renderReportFlagSection(reportId, existingFlag) {
  ...
  contentEl.innerHTML = `<button type="button" class="settings-btn" id="flagReportToggleBtn" onclick="showFlagReportForm('${reportId}')"><img class="icon-img icon-img-inline" src="icons/flag.png" alt="">${t('flagReportBtn')}</button>`;
}
...
function renderContactRows(c, reportId, isFixed) {
  ...
  const otherFlagBtn = c.id != null
    ? `<button type="button" class="contact-flag-btn contact-flag-other-btn" onclick="toggleContactOtherFlagForm('${companyIdAttr}')"><img class="contact-card-icon" src="icons/flag.png" alt="">${t('contactFlagOtherBtn')}</button>` : '';
  const phones = contactEntries(c.phone);
  const emails = contactEntries(c.email);
  // Once the report is fixed there's nothing left to call or email the
  // company *about this report* — so the prominent action buttons go away —
  // but the phone/email themselves are still real contact info for the
  // company, so they fall back to plain (untracked) tel:/mailto: links
  // rather than losing clickability entirely. Website, address, notes, and
  // "wrong info" flagging stay active — those are properties of the
  // company, not this report.
  // Buttons show the actual number/address (with its label prefix, when
  // set) rather than a generic "Call"/"Send" caption — with multiple
  // contacts per company (or multiple companies) that's the only way to
  // tell the buttons apart at a glance.
  const callButtonsHtml = !isFixed ? phones.map(p => `
    <a class="contact-action-btn contact-call-btn" href="tel:${escapeHtml(p.value)}"${phoneClickAttr}><img class="icon-img icon-img-inline" src="icons/phone.png" alt="">${p.label ? escapeHtml(p.label) + ': ' : ''}${escapeHtml(p.value)}</a>`).join('') : '';
  // Every button still triggers the exact same emailReport() flow as
  // before — one preset-body email addressed to every contact for this
  // report, not just this company/address — just labeled with the address
  // it's associated with instead of a generic caption. That flow needs a
  // reportId (it reads #reportDetailBody fields and sends on behalf of a
  // specific report), so it only applies inside the report-detail modal.
  // Elsewhere — e.g. the municipality contacts modal, which renders these
  // same cards with reportId=null just to browse who to reach out to —
  // fall back to a plain mailto: link instead of dropping email entirely
  // (phones already get this same plain-link treatment via the tel: href
  // above, which never required a reportId either).
  const emailButtonHtml = !isFixed ? emails.map(e => reportId
    ? `<button type="button" class="contact-action-btn contact-email-btn" onclick="emailReport('${escapeHtml(reportId)}')"><img class="icon-img icon-img-inline" src="icons/email.png" alt="">${e.label ? escapeHtml(e.label) + ': ' : ''}${escapeHtml(e.value)}</button>`
    : `<a class="contact-action-btn contact-email-btn" href="mailto:${escapeHtml(e.value)}"><img class="icon-img icon-img-inline" src="icons/email.png" alt="">${e.label ? escapeHtml(e.label) + ': ' : ''}${escapeHtml(e.value)}</a>`
  ).join('') : '';
  const actionRowHtml = (callButtonsHtml || emailButtonHtml)
    ? `<div class="contact-action-row">${callButtonsHtml}${emailButtonHtml}</div>` : '';
  const phoneRowsHtml = isFixed ? phones.map(p =>
    `<div class="contact-card-row"><img class="contact-card-icon" src="icons/phone.png" alt="phone"><a href="tel:${escapeHtml(p.value)}">${p.label ? `<span class="contact-card-entry-label">${escapeHtml(p.label)}</span>` : ''}${escapeHtml(p.value)}</a></div>`).join('') : '';
  const emailRowsHtml = isFixed ? emails.map(e =>
    `<div class="contact-card-row"><img class="contact-card-icon" src="icons/email.png" alt="email"><a href="mailto:${escapeHtml(e.value)}">${e.label ? `<span class="contact-card-entry-label">${escapeHtml(e.label)}</span>` : ''}${escapeHtml(e.value)}</a></div>`).join('') : '';
  return `
      ${actionRowHtml}
      ${phoneRowsHtml}
      ${emailRowsHtml}
...
// Copies just the plain-text location (street, area, municipality) shown
// in the detail panel's location row — same copy→check→copy icon swap as
// copyReportLinkToClipboard, no toast, since the icon change is feedback
// enough.
let reportLocationCopyIconRevertTimer = null;
async function copyReportLocationToClipboard() {
  const streetEl = document.getElementById('detailStreetValue');
  const areaEl = document.getElementById('detailAreaValue');
  const muniEl = document.getElementById('detailMunicipalityValue');
  const parts = [streetEl, areaEl, muniEl].map(el => el ? el.textContent.trim() : '').filter(Boolean);
  const text = parts.join(', ');
  if (!text) return;
  try {
    await navigator.clipboard.writeText(text);
    const icon = document.getElementById('detailLocationCopyIcon');
    if (icon) {
      icon.src = 'icons/check.png';
      clearTimeout(reportLocationCopyIconRevertTimer);
      reportLocationCopyIconRevertTimer = setTimeout(() => { icon.src = 'icons/copy.png'; }, 5000);
    }
  } catch (err) {
    console.error('Clipboard write failed:', err.message);
    await themedPrompt(t('copyLinkPrompt'), text, { cancelLabel: null, okLabel: 'OK' });
  }
}
...
async function emailReport(reportId) {
  ...
  // One "Posalji email" button per contact card can all trigger this same
  // send — disable them all for the duration so a double-tap (or tapping a
  // second company's button mid-send) can't fire it twice.
  const btns = Array.from(document.querySelectorAll('#reportDetailBody .contact-email-btn'));
...
function buildDetailStatusReadonlyHtml(report) {
  const isAdmin = !!(currentProfile && currentProfile.is_admin);
  const isOwner = !!(currentSession && report.owner_id === currentSession.user.id);

  const hasFullPower = hasFullPowerOverReport(report) || (isAdmin && isOwner);

  // The inline "add photo" icon button that used to live here (and later
  // moved to the header) is gone now — adding a photo only happens from
  // the timeline's "+" action — so this row is just the comment now, when
  // there is one.
  const noteRowHtml = report.comment
    ? `<div class="popup-note-row" style="margin-top:10px;"><div class="popup-note popup-note-full">${escapeHtml(report.comment)}</div></div>`
    : '';
  const personalResolveNoteHtml = (() => {
    // The self-serve "mark in progress"/"mark fixed" buttons themselves
    // now live as clickable dots right on the timeline below (see
    // timelineStageAction/buildTimelineStageActionItemHtml) instead of a
    // separate button row here — this just keeps the one-line context
    // note explaining why the owner gets a fast-track button at all,
    // shown above the timeline instead of stacked under buttons that no
    // longer exist in this card.
    if (hasFullPower || !isOwner || report.status === 'fixed') return '';
    if (reportIsPersonalProblem(report)) {
      return `<div class="popup-personal-note">${t('personalProblemOwnerNote')}</div>`;
    }
    if (report.status === 'reported' && !categorySkipsInProgress(report.category, report.subcategory)) {
      return `<div class="popup-personal-note">${t('ownerInProgressNote')}</div>`;
    }
    return '';
  })();
  // Status + priority as a pill row up top, with the "report an issue with
  // this report" control now folded into the same row (right-aligned,
  // where the row has spare room) instead of its own section further
  // down. Address/location below it, then the (optional) self-serve
  // status-advance button + hint. Edit/delete controls and the timeline
  // used to live in this same card — they now have their own spots
  // (header icon / bottom admin section, and their own "STATUS PRIJAVE"
  // card) so this one stays a compact overview.
  const canFlagReport = !!(currentSession && !isOwner);
  const flagInlineHtml = canFlagReport ? `
    <div class="detail-flag-inline" id="reportFlagSection">
      <div id="reportFlagContent"><div class="detail-loading">${t('detailLoading')}</div></div>
    </div>` : '';
  return `
    <div class="detail-badge-row">
      <div class="detail-badge-pills">
    ...
    <div class="detail-location-row">
      <div class="detail-location-label"><img class="icon-img" src="icons/pin.png" alt=""></div>
      ...
      <button type="button" class="detail-location-copy-btn" onclick="copyReportLocationToClipboard()" id="detailLocationCopyBtn" aria-label="${tOr('copyAddressBtn', 'Copy address')}" title="${tOr('copyAddressBtn', 'Copy address')}"><img class="icon-img" id="detailLocationCopyIcon" src="icons/copy.png" alt="copy location"></button>
    </div>
    ...
    ${personalResolveNoteHtml}
    ${noteRowHtml}
  `;
}

// The bottom-of-timeline "add a photo" action — same dot/line rhythm as
// every other entry, but a dashed hollow ring holding a plain "+" instead
// of a status/event icon, and no date (it's an available action, not
// something that already happened). Clicking it re-uses the existing
// camera/gallery chooser modal (addTimelinePhoto figures out afterward
// which slot the picked photo actually belongs in).
function buildTimelineAddPhotoItemHtml(reportId) {
  const label = escapeHtml(t('photoSourceTitle') || 'Add a photo');
  return `<div class="timeline-item timeline-action timeline-add-photo">
    <div class="timeline-line"></div>
    <div class="timeline-dot" role="button" tabindex="0" aria-label="${label}" onclick="addTimelinePhoto('${reportId}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();addTimelinePhoto('${reportId}');}"><span class="timeline-add-photo-plus">+</span></div>
    <div class="timeline-content">
      <span class="timeline-action-label" role="button" tabindex="0" onclick="addTimelinePhoto('${reportId}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();addTimelinePhoto('${reportId}');}">${label}</span>
    </div>
  </div>`;
}

// What clicking a not-yet-reached "in progress"/"fixed" timeline dot
// should do, if anything — the same three routes the map-popup's status
// buttons already offer (direct set for full power, self-serve resolve
// for an owner on their own personal-problem/fast-track report, otherwise
// a community vote), just surfaced right on the timeline dot instead of a
// separate button row. Returns null when nobody can act on this stage
// right now (not signed in, report already fixed, category skips
// in_progress, etc.) — that dot then stays a plain, non-interactive
// placeholder.
function timelineStageAction(report, key) {
  if (!currentSession) return null;
  if (report.status === key || report.status === 'fixed') return null;
  if (key === 'in_progress' && categorySkipsInProgress(report.category, report.subcategory)) return null;
  const isOwner = currentSession.user.id === report.owner_id;
  const isAdmin = !!(currentProfile && currentProfile.is_admin);
  const hasFullPower = hasFullPowerOverReport(report) || (isAdmin && isOwner);
  const statusLabelText = key === 'in_progress' ? t('markInProgress') : t('markFixed');
  if (hasFullPower) {
    return { label: statusLabelText, handler: `updateReportStatus('${report.id}','${key}')`, isVote: false };
  }
  const isPersonal = reportIsPersonalProblem(report);
  if (isPersonal) {
    // Personal problems (low water pressure, no power, a gas smell, etc.)
    // are the reporter's own to resolve — not something a stranger can
    // vote on, and not something that needs the in-progress step either:
    // the reporter is the only one who'd actually know their water's back
    // on, so they can self-mark straight to "fixed" any time, whether or
    // not they (or anyone else) ever logged it as in-progress first. Same
    // rule the compact popup follows.
    if (!isOwner) return null;
    return { label: statusLabelText, handler: `resolvePersonalReportFromDetail('${report.id}','${key}')`, isVote: false };
  }
  // Everyone else's own report (a public pothole, a broken streetlight,
  // etc.) still can't be self-marked "fixed" — only voted on like anyone
  // else's — even though "in progress" gets a fast-track below. Public
  // issues benefit from someone other than the reporter confirming it's
  // actually done; a private utility problem has no one else who could.
  const isOwnerFastTrack = isOwner && key === 'in_progress' && report.status === 'reported';
  if (isOwnerFastTrack) {
    return { label: statusLabelText, handler: `resolvePersonalReportFromDetail('${report.id}','${key}')`, isVote: false };
  }
  // If this is the status the user already voted for, show it as an
  // "already voted" state instead of a plain vote button — tapping it
  // again retracts the vote (covers a mis-tap) rather than re-casting the
  // same vote, which the RPC would just silently no-op on anyway.
  if (myVoteByReport.get(report.id) === key) {
    const votedLabel = `${tOr('timelineAlreadyVotedPrefix', 'Already voted')}: ${statusLabelText}`;
    return { label: votedLabel, handler: `retractStatusVoteFromTimeline('${report.id}')`, isVote: true, key, alreadyVoted: true };
  }

  const voteLabel = `${tOr('timelineVoteBtnPrefix', 'Vote')}: ${statusLabelText}`;
  return { label: voteLabel, handler: `castStatusVoteFromTimeline('${report.id}','${key}')`, isVote: true, key };
}

// Same "N out of VOTE_THRESHOLD votes so far" wording the map-popup's
// buildVoteProgressHtml badge shows (t('voteProgress'), e.g. "U toku?
// (1/3 glasa)") — just surfaced as a second line right under the
// timeline's own inline vote action instead of in a separate badge, so a
// voter sees the same count wherever they cast the vote. Only applies to
// actual vote actions (isVote) and only once someone's actually voted;
// direct-power/self-serve actions and untouched stages stay unlabeled.
function buildTimelineVoteProgressLabelHtml(report, key) {
  const progress = voteProgressByReport.get(report.id);
  const p = progress && progress[key];
  if (!p || p <= 0 || p >= VOTE_THRESHOLD) return '';
  const text = t('voteProgress').replace('{status}', statusLabel(key)).replace('{p}', p);
  return `<span class="timeline-note timeline-vote-progress">${escapeHtml(text)}</span>`;
}

// Renders a pending in_progress/fixed dot as a clickable action — same
// dashed/hover treatment as the "add a photo" dot (.timeline-action in
// CSS), keeping the stage's own icon (hourglass/thumbs-up) instead of a
// "+", with the action's button text as its label.
function buildTimelineStageActionItemHtml(report, key, action) {
  const icon = STAGE_ICONS[key];
  const label = escapeHtml(action.label);
  const handler = action.handler;
  const keydown = `if(event.key==='Enter'||event.key===' '){event.preventDefault();${handler};}`;
  const voteProgressHtml = action.isVote ? buildTimelineVoteProgressLabelHtml(report, key) : '';
  // Two lines (action label + vote count) need the same reduced top
  // margin every other two-line timeline row uses to stay centered on
  // the dot — see .timeline-content.has-label.
  const contentClass = voteProgressHtml ? 'timeline-content has-label' : 'timeline-content';
  // Admin-only "clear votes" control, right-aligned next to the action
  // label — same ✕ treatment as the per-vote delete on individual
  // "Suggested: ..." rows further down the timeline, just scoped to wipe
  // every outstanding vote for this suggested status in one go instead
  // of one at a time, for whenever the tally looks stuck or wrong.
  const clearVotesHtml = (action.isVote && currentProfile && currentProfile.is_admin)
    ? timelineExtraDeleteBtnHtml(`clearTimelineStatusVotes('${report.id}','${key}')`)
    : '';
  const labelRowHtml = clearVotesHtml
    ? `<span class="timeline-action-row"><span class="timeline-action-label" role="button" tabindex="0" onclick="${handler}" onkeydown="${keydown}">${label}</span>${clearVotesHtml}</span>`
    : `<span class="timeline-action-label" role="button" tabindex="0" onclick="${handler}" onkeydown="${keydown}">${label}</span>`;
  const alreadyVotedClass = action.alreadyVoted ? ' timeline-already-voted' : '';
  return `<div class="timeline-item pending timeline-action${action.isVote ? ' timeline-vote-action' : ''}${alreadyVotedClass}">
    <div class="timeline-line"></div>
    <div class="timeline-dot" role="button" tabindex="0" aria-label="${label}" onclick="${handler}" onkeydown="${keydown}"><img class="timeline-dot-icon" src="${icon}" alt=""></div>
...
// Whether there's any photo left to add for this report right now, and if
// so which slot it belongs in — same eligibility rules the old separate
// before/after/gallery add-buttons used to gate on individually, just
// checked together so the single timeline "+" knows where to route
// whatever the person picks.
function timelineAddPhotoTarget(report) {
  if (!currentSession) return null;
  const isOwner = currentSession.user.id === report.owner_id;
  const isAdmin = !!(currentProfile && currentProfile.is_admin);
  const isFixed = report.status === 'fixed';
  // Only replaces/creates the single "before" evidence slot when there
  // isn't already a valid one sitting there (nothing yet, or the last
  // attempt was rejected) — this used to only apply to owners; an admin
  // hit an unconditional `|| isAdmin` here that skipped the check
  // entirely, so adding a photo as an admin always overwrote whatever
  // "before" photo already existed instead of adding alongside it. Now
  // both roles follow the same rule, and once a valid photo is already
  // in place, any further add falls through to the gallery below instead.
  const canReplaceBefore = (isOwner || isAdmin) && (!report.photo_path || report.photo_status === 'rejected');
  if (!isFixed && canReplaceBefore) return 'before';
  if (isFixed && !report.after_photo_path && (isOwner || isAdmin)) return 'after';
  if (!isFixed) return 'gallery';
  return null;
}

// Routes a photo picked from the timeline's "+" action to whichever slot
// currently makes sense for this report (see timelineAddPhotoTarget) —
// the main "before" evidence photo, the "after" photo once fixed, or a
// community gallery upload otherwise.
function addTimelinePhoto(reportId) {
  const report = globalActiveData.find(r => r.id === reportId);
  if (!report) return;
  const target = timelineAddPhotoTarget(report);
  if (target === 'before') addPhotoToReport(reportId);
  else if (target === 'after') addAfterPhotoToReport(reportId);
  else if (target === 'gallery') addGalleryPhotoToReport(reportId);
}

// Casts a status vote from the timeline's inline "vote" dot, then
// refreshes the open detail modal so the vote — or a resulting status
// change, if this pushed it over the threshold — shows up immediately.
// castStatusVote itself only refreshes the pins/profile; this just also
// redraws whichever detail modal is actually open right now.
async function castStatusVoteFromTimeline(reportId, suggestedStatus) {
  await castStatusVote(reportId, suggestedStatus);
  await showReportDetailModal(reportId);
}

// Undoes the current user's own pending status vote — the timeline's vote
// dot routes here instead of castStatusVoteFromTimeline once it detects
// (via myVoteByReport) that this is the status they already voted for, so
// tapping it a second time reads as "undo my mistake" rather than casting
// the same vote again.
async function retractStatusVoteFromTimeline(reportId) {
  if (!currentSession) { toast(t('signInFirst'), 'error'); return; }
  try {
    const { error } = await sb.rpc('retract_status_vote', { p_report_id: reportId });
    if (error) throw error;
    toast(tOr('voteRemoved', 'Vote removed'), 'success');
    await loadProfile();
    await loadPinsByWindow();
  } catch (err) {
    console.error('Vote retract error (full):', err);
    toast(t('voteFail'), 'error');
  }
  await showReportDetailModal(reportId);
}

// The pipeline timeline now lives in its own "STATUS PRIJAVE" card, separate
// from the compact status overview above. The "reported" entry additionally
// carries who reported it (name + admin ban control, when applicable) —
// previously a separate detail-row, now folded into its timeline entry.
function buildDetailTimelineCardHtml(report, reporterName) {
  // Every pipeline stage (reported/in progress/fixed) shows its plain
  // status name on the left — same caption style as every other timeline
  // label, never the colored pill (that's reserved for the current-status
  // badge up in the compact overview) — including stages that haven't
  // happened yet, so the full reported → in progress → fixed sequence is
  // always legible, not just the reached steps. The "reported" row is the
  // one exception: instead of the plain stage name it reads "Reported by
  // [name]" (reusing the same reportedByLabel string the map-popup row
  // already uses), with the reporter's username/id and, for admins, the
  // ban control folded right in — same info as before, just phrased as
  // one sentence instead of a label/value pair split across the row.
  const reporterRowHtml = `<span class="timeline-reporter-row">${escapeHtml(t('reportedByLabel'))} <span class="timeline-reporter-name">${escapeHtml(reporterName)}</span>${reporterBanControlHtml(report)}</span>`;
  const stageLabelHtml = (key) => `<span class="timeline-stage-label">${statusLabel(key)}</span>`;
  return `
    <div class="detail-section-title">${t('popupStatus')}</div>
    <div id="reportDetailStatusSection">
      ${buildDetailStatusReadonlyHtml(report)}
    </div>
    <div class="popup-timeline" id="detailTimeline-${report.id}">
      ${buildTimelineItem(report.created_at, true, STATUS_COLORS.reported, reporterRowHtml, STAGE_ICONS.reported)}
      ${(!categorySkipsInProgress(report.category, report.subcategory) && report.in_progress_at) ? buildTimelineItem(report.in_progress_at, true, STATUS_COLORS.in_progress, stageLabelHtml('in_progress'), STAGE_ICONS.in_progress) : ''}
      ${report.fixed_at ? buildTimelineItem(report.fixed_at, true, STATUS_COLORS.fixed, stageLabelHtml('fixed'), STAGE_ICONS.fixed) : ''}
      <span id="detailTimelineAnchor-${report.id}"></span>
      ${timelineAddPhotoTarget(report) ? buildTimelineAddPhotoItemHtml(report.id) : ''}
      ${(!categorySkipsInProgress(report.category, report.subcategory) && !report.in_progress_at && !report.fixed_at) ? (() => {
          const action = timelineStageAction(report, 'in_progress');
          return action
            ? buildTimelineStageActionItemHtml(report, 'in_progress', action)
            : buildTimelineItem(report.in_progress_at, false, STATUS_COLORS.in_progress, null, STAGE_ICONS.in_progress);
        })() : ''}
      ${!report.fixed_at ? (() => {
          const action = timelineStageAction(report, 'fixed');
          return action
            ? buildTimelineStageActionItemHtml(report, 'fixed', action)
            : buildTimelineItem(report.fixed_at, false, STATUS_COLORS.fixed, null, STAGE_ICONS.fixed);
        })() : ''}
    </div>
  `;
}

// Full-power (admin) edit/delete controls, now shown as their own section
// at the bottom of the panel instead of icon buttons up in the status card.
// A plain owner's own lightweight delete lives as an icon button in the
// header instead (wired in showReportDetailModal) — this section is
// admin-only and renders nothing otherwise.
function buildDetailAdminActionsHtml(report) {
  const isAdmin = !!(currentProfile && currentProfile.is_admin);
  const isOwner = !!(currentSession && report.owner_id === currentSession.user.id);
  const hasFullPower = hasFullPowerOverReport(report) || (isAdmin && isOwner);
  if (!hasFullPower) return '';
  return `
...
// Improvement #4: a compact, admin-only record of what the AI moderator
// decided vs. what a human admin ultimately did, per photo slot on this
// report. Built entirely from columns already on brake_reports (hydrated
// by ensureFullReportDetail via REPORT_DETAIL_ONLY_FIELDS) — no new table.
// Reviewer usernames are resolved afterwards by loadModerationHistoryReviewers()
// since profiles lookups shouldn't block the initial render.
const MODERATION_VERDICT_COLOR = { approved: UI_COLORS.success, rejected: UI_COLORS.dangerStrong, flagged: UI_COLORS.dangerStrong };

function buildModerationHistoryRow(opts) {
  const { labelKey, labelFallback, uploadedAt, verdict, confidence, reason, reviewedAt, reviewedBy, finalStatus, rowId } = opts;
  if (!uploadedAt && !verdict && !reviewedAt) return '';
  const verdictHtml = verdict
    ? `<span class="photo-status-badge" style="background:${MODERATION_VERDICT_COLOR[verdict] || 'var(--text-muted)'};">${escapeHtml(tOr('modHistoryAiVerdict', 'AI: {verdict}').replace('{verdict}', verdict))}${confidence ? ` (${escapeHtml(confidence)})` : ''}</span>`
    : `<span class="photo-status-badge" style="background:var(--text-muted);">${escapeHtml(tOr('modHistoryNoAiVerdict', 'No AI verdict'))}</span>`;
  const finalHtml = finalStatus
    ? `<span class="photo-status-badge" style="background:${MODERATION_VERDICT_COLOR[finalStatus] || 'var(--text-muted)'};">${escapeHtml(tOr('modHistoryFinal', 'Final: {status}').replace('{status}', finalStatus))}</span>`
    : '';
  return `
    <div class="report-flag-history-item" id="modhist-row-${rowId}">
      <div class="photo-review-title">${escapeHtml(tOr(labelKey, labelFallback))} ${verdictHtml} ${finalHtml}</div>
      ${reason ? `<div class="report-flag-history-meta">${escapeHtml(tOr('modHistoryReason', 'Reason: {reason}').replace('{reason}', reason))}</div>` : ''}
      <div class="report-flag-history-meta">
        ${uploadedAt ? escapeHtml(tOr('modHistorySubmitted', 'Submitted {date}').replace('{date}', formatDate(uploadedAt))) : ''}
        ${reviewedAt ? ' · ' + escapeHtml(tOr('modHistoryReviewed', 'Reviewed {date}').replace('{date}', formatDate(reviewedAt))) + (reviewedBy ? ` <span id="modhist-reviewer-${rowId}"></span>` : '') : ''}
      </div>
    </div>`;
}

function buildModerationHistoryHtml(report) {
  const rows = [
    buildModerationHistoryRow({
      rowId: 'main', labelKey: 'modHistoryMainPhoto', labelFallback: 'Before photo',
      uploadedAt: report.photo_uploaded_at, verdict: report.photo_moderation_verdict,
      confidence: report.photo_moderation_confidence, reason: report.photo_moderation_reason,
      reviewedAt: report.photo_reviewed_at, reviewedBy: report.photo_reviewed_by,
      finalStatus: report.photo_status
    }),
    buildModerationHistoryRow({
      rowId: 'after', labelKey: 'modHistoryAfterPhoto', labelFallback: 'After photo',
      uploadedAt: report.after_photo_uploaded_at, verdict: report.after_photo_moderation_verdict,
      confidence: report.after_photo_moderation_confidence, reason: report.after_photo_moderation_reason,
      reviewedAt: report.after_photo_reviewed_at, reviewedBy: report.after_photo_reviewed_by,
      finalStatus: report.after_photo_status
    }),
    buildModerationHistoryRow({
      rowId: 'fixed', labelKey: 'modHistoryFixedPhoto', labelFallback: 'Fixed photo',
      uploadedAt: report.fixed_photo_uploaded_at, verdict: null, confidence: null, reason: null,
      reviewedAt: report.fixed_photo_reviewed_at, reviewedBy: report.fixed_photo_reviewed_by,
      finalStatus: report.fixed_photo_status
    })
  ].filter(Boolean);
  if (!rows.length) return '';
  return `
    <div class="detail-section-title">${escapeHtml(tOr('modHistoryTitle', 'Moderation history'))}</div>
    <div class="report-flag-history">${rows.join('')}</div>
  `;
}

async function loadModerationHistoryReviewers(report) {
  const ids = Array.from(new Set([report.photo_reviewed_by, report.after_photo_reviewed_by, report.fixed_photo_reviewed_by].filter(Boolean)));
  if (!ids.length) return;
  try {
    const { data, error } = await sb.from(PROFILES_TABLE).select('id, username').in('id', ids);
    if (error) throw error;
    const usernames = new Map((data || []).map(p => [p.id, p.username]));
    [['main', report.photo_reviewed_by], ['after', report.after_photo_reviewed_by], ['fixed', report.fixed_photo_reviewed_by]].forEach(([rowId, uid]) => {
      if (!uid) return;
      const el = document.getElementById(`modhist-reviewer-${rowId}`);
      if (el) el.textContent = tOr('modHistoryBy', 'by {name}').replace('{name}', usernames.get(uid) || t('detailUnknown'));
    });
  } catch (err) {
    console.error('Failed to resolve moderation reviewer usernames:', err.message);
  }
}
...
function buildDetailEditFieldsHtml(report) {
  ...
  ].filter(s => s.key !== 'in_progress' || !categorySkipsInProgress(report.category, report.subcategory));
  ...
  return `
    ...
    <div class="detail-edit-field">
      ...
      <select id="detailEditSubcat-${report.id}" onchange="onDetailEditSubcategoryChange('${report.id}')"></select>
    </div>
    <div class="detail-edit-field">
...
// Rebuilds the status <select>'s options for the edit form's current
// category+subcategory pick, preserving whatever status is still
// selected if it's still a valid choice (e.g. switching between two
// categories that both keep "in progress" shouldn't reset the status
// back to "reported"), falling back to "reported" only when the
// currently-selected status just got filtered out (picking a category/
// subcategory pair that skips "in progress" while "in progress" was
// selected). Shared by both the category-select and subcategory-select
// onchange handlers below, since either one can flip whether "in
// progress" applies (see SUBCATEGORIES_SKIPPING_IN_PROGRESS).
function refreshDetailEditStatusOptions(reportId, category, subcategory) {
  ...
  const statuses = [
    ...
    { key:'in_progress', label: statusLabel('in_progress') },
    { key:'fixed', label: statusLabel('fixed') }
  ].filter(s => s.key !== 'in_progress' || !categorySkipsInProgress(category, subcategory));
...
function onDetailEditCategoryChange(reportId) {
  ...
  // A category switch always clears the subcategory pick (the old
  // subcategory almost certainly doesn't exist under the new category),
  // so the status refresh here only needs to consider the category on
  // its own — same as passing subcategory=null.
  populateSubcategoryOptions(subSelect, catSelect.value, null);
  refreshDetailEditStatusOptions(reportId, catSelect.value, null);
}

// Subcategory on its own can also flip whether "in progress" applies
// (see SUBCATEGORIES_SKIPPING_IN_PROGRESS) even when the category
// didn't change — e.g. picking "Illegal logging" under Forest after
// already having Forest selected. No subcategory-options repopulation
// needed here (only the category select does that); just re-filter the
// status select for the newly-picked subcategory.
function onDetailEditSubcategoryChange(reportId) {
  const catSelect = document.getElementById(`detailEditCat-${reportId}`);
  const subSelect = document.getElementById(`detailEditSubcat-${reportId}`);
  if (!catSelect || !subSelect) return;
  refreshDetailEditStatusOptions(reportId, catSelect.value, subSelect.value || null);
}
...
function cancelReportDetailEdit(reportId) {
  ...
  const section = document.getElementById('reportDetailStatusSection');
  if (!report || !section) return;
  section.innerHTML = buildDetailStatusReadonlyHtml(report);
}
...
// markers/clustering (municipality_id for admin-domain checks), the
...
// could happen), and now fixed_at (isFixedAgeVisible, so a fixed pin's
// age can be checked on the map regardless of whether it's inside the
// selected date range or has ever been opened in the detail modal):
const REPORT_LIST_COLUMNS =
  ...
  'photo_path,photo_status,after_photo_path,after_photo_status,fixed_at';
// Same as above, plus in_progress_at — needed because the date-range query
// also feeds dateRangeOnlyData, which powers the admin CSV export
// (downloadCSV()). fixed_at itself already lives in REPORT_LIST_COLUMNS now.
const REPORT_DATE_RANGE_COLUMNS = REPORT_LIST_COLUMNS + ',in_progress_at';
...
// Map of municipality_id -> subscribed Realtime channel. Each municipality
// currently represented in globalActiveData (i.e. visible in the loaded
// viewport) gets its own public Broadcast topic, 'reports:muni:<id>',
// populated server-side by the trg_broadcast_report_change trigger (see
// broadcast_report_changes_by_municipality migration). This replaces the
// earlier postgres_changes-based approach: broadcast payloads are hand-built
// lean objects (no `comment`/`path`, the two fields that can actually be
// large) rather than full rows, and delivery is topic-scoped so Realtime
// never even has to run a per-subscriber authorization check the way
// postgres_changes does on every change.
//
// Channels are added/removed incrementally as the visible set changes
// (rather than tearing everything down on every pan) so a small nudge of
// the map doesn't churn already-open, still-relevant subscriptions.
const reportsRealtimeChannels = new Map();
// Safety valve for an extreme zoom-out spanning an unreasonable number of
// municipalities at once — rather than open hundreds of concurrent channel
// joins, cap it and accept partial live coverage for that view (the regular
// viewport-reload debounce still catches anything missed within seconds).
const REALTIME_MUNI_CHANNEL_CAP = 60;

function getVisibleMunicipalityIds() {
  const ids = new Set();
  globalActiveData.forEach(r => { if (r.municipality_id != null) ids.add(r.municipality_id); });
...
function applyRealtimeReportChange(eventType, newRow, oldRow) {
  ...
  // Broadcast payloads are intentionally lean (no comment/path) to keep
  // messages small, so — unlike the old postgres_changes payloads, which
  // always carried every column — this is NOT a full row. Merge onto any
  // existing object (preserving previously-hydrated detail fields) instead
  // of replacing it outright, and leave _detailHydrated alone so
  // ensureFullReportDetail() still fetches the rest on demand if the
  // detail modal gets opened for a report we've only ever seen a lean
  // payload for.
  if (idx === -1) {
    globalActiveData.push(newRow);
  } else {
    Object.assign(globalActiveData[idx], newRow);
  }
  markActiveDataChanged();
...
function subscribeReportsMuniTopic(muniId) {
  const channel = sb.channel('reports:muni:' + muniId, { config: { private: false } })
    .on('broadcast', { event: 'change' }, msg => {
      try {
        const p = msg.payload || {};
        applyRealtimeReportChange(p.eventType, p.new, p.old);
      } catch (err) {
        console.error('Realtime report sync error:', err);
      }
    })
    .subscribe();
  reportsRealtimeChannels.set(muniId, channel);
}

function unsubscribeReportsMuniTopic(muniId) {
  const channel = reportsRealtimeChannels.get(muniId);
  if (!channel) return;
  sb.removeChannel(channel);
  reportsRealtimeChannels.delete(muniId);
}

// Called after every viewport (re)load — diffs the newly-visible
// municipality set against the currently-subscribed one and only
// adds/removes the difference, so unchanged municipalities keep their
// existing channel (and any in-flight messages) untouched.
function refreshReportsRealtimeFilter() {
  let visible = getVisibleMunicipalityIds();

  if (visible.length > REALTIME_MUNI_CHANNEL_CAP) {
    console.warn('[realtime] viewport spans ' + visible.length + ' municipalities — capping live updates to '
      + REALTIME_MUNI_CHANNEL_CAP + '; the rest will only refresh on the next viewport reload.');
    visible = visible.slice(0, REALTIME_MUNI_CHANNEL_CAP);
  }
  const visibleSet = new Set(visible);

  Array.from(reportsRealtimeChannels.keys()).forEach(muniId => {
    if (!visibleSet.has(muniId)) unsubscribeReportsMuniTopic(muniId);
  });
  visible.forEach(muniId => {
    if (!reportsRealtimeChannels.has(muniId)) subscribeReportsMuniTopic(muniId);
  });
}

// Fetches whether the current user already has a pending status vote on
// this report and, if so, which status they voted for — report_status_votes
// is readable by any authenticated user (RLS), just not directly writable,
// so a plain select filtered to our own row is enough here. Called right
// before the detail modal's timeline renders so timelineStageAction can
// use it synchronously.
async function loadMyVoteForReport(reportId) {
  myVoteByReport.delete(reportId);
  if (!currentSession) return;
  try {
    const { data, error } = await sb.from(REPORT_STATUS_VOTES_TABLE)
      ...
      .maybeSingle();
    if (error) throw error;
    if (data) myVoteByReport.set(reportId, data.suggested_status);
  } catch (err) {
    console.error('Failed to load own vote status:', err.message);
  }
}
